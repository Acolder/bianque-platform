...
export default {
  ...
  methods: {
    // 获取申请用户列表数据
    getDataList() {
      axois.post('/api/user/GetUserInfoListInfo', this.userForm).then(res => {
    ...
    conform() {
      ...
      axois.get('/api/user/UpdateUserInfoRemarkByid?id=' + this.id + '&remark=' + this.remark).then(res => {
        if (res.data.code === 0 && res.status === 200) {
          this.$message({
            type: 'success',
    ...
    checkPass(id) {
      let status = 1;
      axois.get('/api/user/ApproveUserInfoByid?id=' + id + '&status=' + status).then(res => {
        if (res.data.code === 0 && res.status === 200) {
          this.$message({
            type: 'success',
            message: res.data.message
