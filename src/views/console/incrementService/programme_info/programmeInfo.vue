<template>
  <div id="programmeWrap">
    <el-form ref="programme" :model="programmeForm" :rules="rules" label-width="130px">
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item label="适宜指标结果:" prop="physique" style="width:90%">
            <el-button icon="el-icon-plus" type="default" size="small" @click="zfShow = true" v-if="zfDetail === ''">请选择指标结果</el-button>
            <el-tag
              :key="tag"
              v-for="tag in tags"
              closable
    ...
    <!-- 选择指标结果 -->
    <el-dialog title="选择指标结果" :visible.sync="zfShow" :modal-append-to-body="false" width="1500px" :append-to-body="true" center>
...
export default {
  data() {
    ...
    return {
      ...
      zfURL: `${process.env.WEB_HOST}/incrementService/PhysicalResultList`,
      getData: {},
      dataParse: '',
      zfListShow: false,
    };
  ...
  methods: {
    ...
    getDetail() {
      ...
      ajax.get(`/api/borrow/GetPhysiotherapyProgrammeInfoByid/` + id).then((res) => {
        if (res.status === 200 && res.data.code === 0) {
          ...
          if (_self.programmeForm.physique) {
            let arr = _self.programmeForm.physique.split(',');
            _self.tags = arr;
          }
        }
      });
    ...
    handleClose(tag) {
      ...
      let physiqueArr = _self.tags.join(',');
      _self.programmeForm.physique = physiqueArr;
    },
    // 选择适宜指标结果
    zfList(data, false1) {
      let _self = this;
