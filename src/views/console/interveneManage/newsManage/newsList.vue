<template>
  <div>
    ...
    <!-- 查看详情 -->
    <el-dialog title="资讯详情" :visible.sync="newsDetail" width="60%" :center="true" :append-to-body="true" :modal-append-to-body="false" class="details">
      <el-row>
        <el-col :span="8"><div class="grid-content bg-purple"><span class="strong">资讯标题：</span><i class="title">{{detailData.title}}</i></div></el-col>
        <el-col :span="8">
          <!-- <el-form-item label="资讯图片：" :prop="'banner_picurl'" >
          ...
          <div class="grid-content bg-purple"><span class="strong">资讯图片：</span>
            <div v-if="detailData.picurl">
              <uploadImg :imgUrl="detailData.picurl" :fileName="'picurl'" class="upImg1"></uploadImg>
              <el-input class="left" type="hidden" v-model="detailData.picurl"></el-input>
            </div>
            <!-- <img :src="detailData.mainimage" alt="" style="width:50px;height:50px;"> -->
          </div>
        </el-col>
        <el-col :span="8"><div class="grid-content bg-purple"><span class="strong">来源名称：</span><i class="title">{{detailData.sourcename}}</i></div></el-col>
      </el-row>
      <el-row>
      ...
      <el-row>
        ...
        <el-col :span="8"><div class="grid-content bg-purple"><span class="strong">摘要：</span><i class="title">{{detailData.abstract}}</i></div></el-col>
...
export default {
  ...
  methods: {
    ...
    // 新增
    addNews() {
      // this.$router.push('/interveneManage/newsManage/addNews');
      this.$router.push({
    ...
    getColumns() {
      ...
      let columns = [
        ...
        { field: 'operation', title: '操作', width: 300, titleAlign: 'center', columnAlign: 'center', componentName: 'table-operation-newsList', isResize: true }
      ];
    ...
    customCompFunc(params) {
      ...
      } else if (params.type === 'down') { // 上架
        if (params.rowData.statusflag === 0 || params.rowData.statusflag === 2) {
          this.$confirm('是否下架: ( ' + params.rowData.title + '  ) 这条资讯?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            this.down([params.rowData.id.toString()]);
          }).catch(() => {
            this.$message({
              type: 'info',
              message: '已取消发布'
    ...
    down(Ids) {
      let _self = this;
      ajax.post(`api/admin/cancelAudit`, Ids).then(res => {
        // console.log(res);
        if (res.status === 200 && res.data.code === 0) {
          _self.$message({
            message: res.data.message,
    ...
    remove(data) {
      // console.log(data);
      let _self = this;
      ajax.post('/api/admin/deleteNews', data).then(res => {
        if (res.status === 200 && res.data.code === 0) {
          _self.$message({
            message: res.data.message,
            type: 'success'
