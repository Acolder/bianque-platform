...
export default {
  data() {
    return {
      ...
      // newsShow: false,
      // productShow: false,
      // ProductsUrl: `${process.env.WEB_HOST}/interveneManage/productOne`,
      // NewListURL: `${process.env.WEB_HOST}/interveneManage/newsManage/newsComponent`,
      // productMiddle: [],
      returnPageIndex: '',  // 页码
      // productPicData: {
      //   proPic: '',  // 更改商品图片
  ...
  computed: {
    ...
    //     { field: 'operation', title: '操作', width: 250, titleAlign: 'center', columnAlign: 'center', componentName: 'table-operations-sc', isResize: true }
    //   ];
  ...
  methods: {
    ...
    */
    ...
    //       this.form.recommendlist = this.subTableData;
    //       this.$message({
    //         type: 'success',
    //         message: '删除成功'
    //       });
    ...
    modifybefore() {
      ...
      ajax.get(`api/admin/getNewsDetail/` + param).then(res => {
        console.log(res);
        if (res.status === 200 && res.data.code === 0) {
    ...
    //     let newsId = _self.form.recommendlist.map(function (item, index, data) {
    //       return item.id;
    //     });
    ...
    //       // console.log(_self.form.recommendlist);
    //     } else {
    //       _self.$message({
    //         message: '该资讯已存在列表，请重新选择',
    //         type: 'warning'
    //       });
    //     }
    //   } else {
    ...
    //       product_main_image: data.pro_pic,
    //       product_category: '1',
    //       product_markprice: data.pro_price,
    //       product_describe: data.pro_describe,
    //       goods_norms: data.pro_size,
    //     };
  ...
  created() {
    ...
    // window.oneNews = this.oneNews;  // 资讯
    // window.oneProduct = this.oneProduct;  // 商品
  }
};
</script>
